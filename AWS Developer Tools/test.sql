...
-- Query to fetch the change ticket details of servers which needs to be cancelled
    ...
    .then(listStateMachines => {
      ...
      for (let i = 0; i < listStateMachines.stateMachines.length; i++) {
        ...
        if (item.name.indexOf(stateMachineName) >= 0) {
          console.log(`Found the Step Machine : ${item}`)
...
module.exports.imageResizer = async (event, context, callback) => {
